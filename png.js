...
function pngPreview(){const preview=$('#sharePreview');preview.innerHTML='';pngBuild();const layout=PNG_LAYOUTS[pngLayout],clone=$('#shareCard').cloneNode(true);clone.removeAttribute('id');clone.classList.add('preview-card');clone.style.setProperty('--preview-w',layout.w+'px');clone.style.setProperty('--preview-h',layout.h+'px');preview.appendChild(clone);requestAnimationFrame(()=>{const scale=Math.min((preview.clientWidth-24)/layout.w,(preview.clientHeight-24)/layout.h);clone.style.transform=`scale(${scale})`;clone.style.transformOrigin='top left';preview.style.height=Math.min(420,layout.h*scale+24)+'px';});}
